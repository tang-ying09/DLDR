...
parser.add_argument('--sketch_root', default='/var/lib/docker/dataset/render_trans/preprocess/lrs2_sketch128', help='root path for sketches')
parser.add_argument('--face_img_root', default='/var/lib/docker/dataset/render_trans/preprocess/lrs2_face128', help='root path for face frame images')
parser.add_argument('--audio_root', default='/var/lib/docker/dataset/render_trans/preprocess/lrs2_audio', help='root path for audio mel')
parser.add_argument('--landmarks_root', default='/var/lib/docker/dataset/render_trans/preprocess/lrs2_landmarks128', help='root path for landmark mel')
...
def evaluate(model, val_data_loader):
    ...
    for epoch in range(eval_epochs):
        ...
        for step, (T_frame_img, T_frame_sketch, ref_N_frame_img, ref_N_frame_sketch,T_mels) in prog_bar:
            ...
            if T_mels.dim() == 5:  # 如果 T_mels 是 5D 张量 [batch_size, channels, 1, mel_bins, time_steps]
                T_mels = T_mels.squeeze(2)  # 移除第3维，变为 [batch_size, channels, mel_bins, time_steps]
            elif T_mels.dim() == 4:  # 如果 T_mels 是 4D 张量 [batch_size, 1, mel_bins, time_steps]
                T_mels = T_mels.permute(0, 1, 3, 2)  # 调整形状为 [batch_size, 1, time_steps, mel_bins]
            if T_mels.size(2) > 80:
                T_mels = T_mels[:, :, :80, :]
            else:
                T_mels = T_mels
    ...
    with open('loss.txt', 'a') as f:
        ...
        f.write(f'PSNR: {psnr:.3f}\n')
        f.write(f'SSIM: {ssim:.3f}\n')
        f.write(f'FID: {fid:.3f}\n')
        f.write(f'Eval Warp Loss: {eval_warp_loss / count:.6f}\n')
        f.write(f'Eval Gen Loss: {eval_gen_loss / count:.6f}\n')
        f.write('-' * 50 + '\n')
    ...
    while global_epoch < 10000:
        ...
        for step, (T_frame_img, T_frame_sketch, ref_N_frame_img, ref_N_frame_sketch, T_mels) in prog_bar:
            ...
            if T_mels.size(2) > 80:
                ...
                T_mels = T_mels
            # print(f"Step: {step}")
            # print(f"T_frame_img shape: {T_frame_img.shape}")
            # print(f"T_frame_sketch shape: {T_frame_sketch.shape}")
            # print(f"ref_N_frame_img shape: {ref_N_frame_img.shape}")
            # print(f"ref_N_frame_sketch shape: {ref_N_frame_sketch.shape}")
            # print(f"T_mels shape: {T_mels.shape}")
            # # time.sleep(1)  # 添加延迟以观察每批次加载情况
            # print(f"Loading batch {step}")
            model.train()
            disc.train()
            optimizer.zero_grad()
            ...
            with open('loss.txt', 'a') as f:
                f.write(f'Global Step: {global_step}\n')
                f.write(f'Running Warp Loss: {running_warp_loss / (step + 1):.6f}\n')
                f.write(f'Running Gen Loss: {running_gen_loss / (step + 1):.6f}\n')
                f.write('-' * 50 + '\n')
